...
const ongoingMatchResolver = {
  ...
  Mutation: {
    ...
    startQueuedMatch: async (_, { id, courtId: providedCourtId }) => {
      try {
        ...
        // For floating matches (no court), use the provided courtId
        const effectiveCourtId = match.courtId || providedCourtId || null;

        // Validate court availability
        if (effectiveCourtId) {
          const court = await Court.findById(effectiveCourtId);
          if (!court) {
            return {
              ok: false,
              message: 'Court not found',
        ...
        // Assign court to floating match before starting
        if (!match.courtId && effectiveCourtId) {
          match.courtId = effectiveCourtId;
        }

...
const gameResolver = {
  ...
  Mutation: {
    recordGame: async (_, { input }) => {
      try {
        const matchId = input?.matchId?.toString?.();
        const playerIds = [...new Set(input.playerIds.map((id) => id.toString()))];
        const rawWinnerIds = Array.isArray(input?.winnerPlayerIds) ? input.winnerPlayerIds : [];
        const winnerIds = [...new Set(rawWinnerIds.map((id) => id.toString()))];
        ...
        if (!matchId) {
          return {
            ok: false,
            message: 'matchId is required to record a game',
            game: null,
          };
        }

        const matchDoc = await OngoingMatch.findById(matchId);
        ...
        if (matchDoc.queued || !matchDoc.startedAt) {
          return {
            ok: false,
            message: 'Cannot record a game for a queued or not-yet-started match',
            game: null,
          };
        }

        const matchPlayerIds = [...new Set((matchDoc.playerIds || []).map((id) => id.toString()))];
        const samePlayerSet =
          matchPlayerIds.length === playerIds.length &&
          matchPlayerIds.every((id) => playerIds.includes(id));

        if (
          matchDoc.sessionId.toString() !== sessionObjectId.toString() ||
          matchDoc.courtId.toString() !== courtObjectId.toString() ||
          !samePlayerSet
        ) {
          return {
            ok: false,
            message: 'recordGame input does not match the active match state',
            game: null,
          };
        }

        // Winner selection is optional for casual matches.
        // If winners are provided, they must not include all players.
